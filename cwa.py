...
def build_time_label_layers(label_points, layer_name_prefix, point_type, actual_font_path):
    ...
    if actual_font_path and QFontDatabase.addApplicationFont(actual_font_path) != -1:
...
def create_layers_for_typhoon(typhoon, actual_font_path):
    ...
    if analysis_fixes:
        ...
        info_text = (f"{typhoon_name} / {intensity_zh} / {last_fix.get('MaxWindSpeed', '?')} m/s / {last_fix.get('Pressure', '?')} hPa")
        ...
        feat.setGeometry(QgsGeometry.fromPointXY(QgsPointXY(lon, lat)))
        feat.setAttributes([typhoon_name, last_fix.get("DateTime", ""), last_fix.get("MaxWindSpeed", ""), last_fix.get("Pressure", ""), intensity_zh, info_text])
        layer_current.dataProvider().addFeatures([feat])
        layer_current.updateExtents()
    layer_time_leader_analysis, layer_time_label_analysis = build_time_label_layers(analysis_label_points, f"{layer_prefix}_TimeLabel_Analysis", "Analysis", actual_font_path)
    layer_time_leader_forecast, layer_time_label_forecast = build_time_label_layers(forecast_label_points, f"{layer_prefix}_TimeLabel_Forecast", "Forecast", actual_font_path)
    return {
        "name": typhoon_name, "ty_no": ty_no, "analysis": layer_analysis, "forecast": layer_forecast, "line": layer_line,
        "radius15": layer_radius15, "radius25": layer_radius25, "current": layer_current,
        "time_leader_analysis": layer_time_leader_analysis, "time_label_analysis": layer_time_label_analysis,
        "time_leader_forecast": layer_time_leader_forecast, "time_label_forecast": layer_time_label_forecast,
    }
...
def build_intensity_categorized_renderer(point_shape, outline_only=False):
    ...
    for lo, hi, code, label, color in INTENSITY_CATEGORIES:
        if outline_only:
            props = {"name": point_shape, "color": "255,255,255,200", "outline_color": color, "outline_width": "0.8", "size": "3.6"}
        else:
            props = {"name": point_shape, "color": color, "outline_color": "0,0,0,255", "outline_width": "0.3", "size": "3.2"}
        categories.append(QgsRendererCategory(code, QgsMarkerSymbol.createSimple(props), label))
    fallback = QgsMarkerSymbol.createSimple({"name": point_shape, "color": "180,180,180,255", "outline_color": "0,0,0,255", "outline_width": "0.3", "size": "3.0"})
...
def style_radius_layer(layer, color_rgba, label):
    ...
    categories = [
        QgsRendererCategory("Analysis", QgsFillSymbol.createSimple({"color": color_rgba, "outline_color": base_rgb + ",180", "outline_width": "0.4"}), f"{label}(現在)"),
        QgsRendererCategory("Forecast", QgsFillSymbol.createSimple({"color": base_rgb + ",35", "outline_color": base_rgb + ",150", "outline_width": "0.4", "outline_style": "dash"}), f"{label}(預測)"),
    ]
...
def handle_font():
    ...
    if os.path.exists(local_path):
        return local_path
    url = "https://github.com/justfont/open-huninn-font/raw/refs/heads/master/font/jf-openhuninn-2.1.ttf"
    try:
        print(f"正在從 {url} 下載字型...")
...
def main():
    ...
    for typhoon in typhoons:
        info = create_layers_for_typhoon(typhoon, actual_font_path)
